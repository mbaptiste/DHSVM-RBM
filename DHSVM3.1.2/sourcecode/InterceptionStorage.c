			 ...
			 float LD_FallVelocity)
{
  ...
  /* The precipitation is multiplied by the fractional coverage, since if the 
     vegetation covers only 10% of the grid cell, only 10% can be intercepted  
     as a maximum */
  for (i = 0; i < NAct; i++) {
    Available = MaxInt[i] - Int[i];
    if (Available > *Precip * Fract[i] * (1 - ImpervFrac))
      Intercepted = (*Precip) * Fract[i] * (1 - ImpervFrac);
    else
  ...
  if(Understory) 
    ...
    *MomentSq = (pow(LD_FallVelocity * WATER_DENSITY, 2) * PI/6 *
		        pow(LEAF_DRIP_DIA, 3) * (*Precip)/Dt) + (1-Fract[0]) * MS_Rainfall;  
}
