		       ...
		       PIXRAD *TotalRad, CHANNEL *ChannelData, float **skyview)
{
    ...
    for (i = 1; i <= 17; i++ ) {           
      if (VType->Height[0] < CanopyHeight[i]) {
        LD_FallVelocity = ((VType->Height[0] - CanopyHeight[i-1])
			   *FallVelocity[i] + (CanopyHeight[i] - VType->Height[0])*FallVelocity[i-1]) /
  ...
  if (VType->OverStory == TRUE &&
    ...
    SnowInterception(y, x, Dt, VType->Fract[0], VType->LAI[0],
		     ...
		     &(LocalSoil->MeltEnergy), &(LocalPrecip->MomentSq), VType->Height, 
	...
	/* calculate the net precip after deducting the intercepted precip */
    InterceptionStorage(VType->NVegLayers, NVegLActual, VType->MaxInt,
		VType->Fract, VType->ImpervFrac, LocalPrecip->IntRain, 
		&(LocalPrecip->RainFall), &(LocalPrecip->MomentSq), VType->Height, 
		VType->UnderStory, Dt, MS_Rainfall, LD_FallVelocity);
  }
  else {
  ...
  if (LocalSnow->HasSnow || LocalPrecip->SnowFall > 0.0) {
    ...
    LocalSnow->Outflow =
      SnowMelt(y, x, Dt, 2. + Z0_SNOW, 0.f, Z0_SNOW, SnowRa, LocalMet->AirDens,
	       ...
	       &(LocalSnow->TPack), &(LocalSnow->TSurf), &(LocalSoil->MeltEnergy));
  ...
  if (VType->OverStory == TRUE) {
    ...
    EvapoTranspiration(0, Dt, VType->ImpervFrac, LocalMet, NetRadiation, 
    ...
    if (LocalSnow->HasSnow != TRUE && VType->UnderStory == TRUE) {
      ...
      NetRadiation =
		  LocalRad.NetShort[1] + LocalRad.LongIn[1] - VType->Fract[1] * LocalRad.LongOut[1];
      EvapoTranspiration(1, Dt, VType->ImpervFrac, LocalMet, NetRadiation, 
  ...
  /* debug */
  // printf("%.2f imp %.2f F Evap=%.1f\n", VType->ImpervFrac, VType->Fract[0], LocalEvap->ETot*100000);
  /* debug ends */
  ...
  if(InfiltOption == STATIC)
    ...
    if (SurfaceWater > 0.) {
      /* Infiltration is a function of the amount of water infiltrated since the storm started */
  ...
  /* Infiltration is updated in UnsaturatedFlow and accumulated below */
  if ((InfiltOption == DYNAMIC) && (SurfaceWater > 0.)) 
    LocalSoil->InfiltAcc += Infiltration; 
  
  if (HeatFluxOption == FALSE) 
    NoSensibleHeatFlux(Dt, LocalMet, MoistureFlux, LocalSoil);

  /* Save the radiation terms for heat flux uses */
  LocalSoil->NetShortIn = LocalRad.NetShortIn;
  LocalSoil->NetLongIn = LocalRad.PixelLongIn;
#endif
